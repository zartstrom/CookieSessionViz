...
class DisjointSet[A] {
  ...
  def unionInOrder(hi: A, lo: A) = {
    val (hiRoot, loRoot) = (hi.root, lo.root)
      loRoot.parent = hiRoot
  }
  ...
  /**
    * @return List of items of the set that x belongs to.
    */
  def set(x: A): Iterable[A] = {
    parent.keys filter (_.root.entry == x.root.entry)
  }

  /**
    * @return Iterator over roots for every set
    */
  def roots(): Iterable[A] = {
    (parent.keys map (_.root.entry)).toList.distinct
  }
}
